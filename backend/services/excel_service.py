...
services/excel_service.py  (v3.2 — cache e campos opcionais corrigidos)
...
Correções:
  - clear_cache disponível para o excel_router
  - cache atualizado com clear() + extend()
  - acesso ao cache protegido por RLock
  - busca de projeto sem diferenciar maiúsculas/minúsculas
  - leitura segura dos campos opcionais do dashboard
"""
...
def get_cached_items() -> list[ProjFuturoItem]:
    """Retorna uma cópia dos itens atualmente armazenados no cache."""
    with _cache_lock:
        return list(_cached_items)


def get_cached_projects() -> list[ProjetoSummary]:
    """Retorna uma cópia dos projetos atualmente armazenados no cache."""
    with _cache_lock:
...
def clear_cache() -> None:
    """
    Limpa os itens e projetos armazenados no cache.
    """
    _set_cache(
        items=[],
        projects=[],
...
def _safe_float(value: object, default: float = 0.0) -> float:
    """Converte um valor para float e devolve default quando inválido."""
    try:
        if isinstance(value, str):
            ...
            # Aceita decimal com vírgula quando não existe ponto decimal.
...
def _optional_nonnegative_float(value: object) -> Optional[float]:
    """Converte para float não negativo; retorna None quando ausente/inválido."""
...
def _model_fields() -> set[str]:
    """Retorna os campos existentes no schema ProjFuturoItem."""
...
def _read_dataframe(
    ...
    """
    Lê os bytes do arquivo e retorna:
      (dataframe, formato, nome_da_aba, linha_do_cabeçalho)
    """
    ...
    if extension == "csv":
        ...
        try:
            ...
            raise ValueError(
                f"Não foi possível ler o arquivo CSV: {exc}"
            ) from exc

    raise ValueError(
        f"Formato '{extension}' não suportado. Use .xlsx, .xls ou .csv."
    )
...
def _build_projects(
    ...
    for item in items:
        project_name = (item.projeto or "Sem Projeto").strip()
        grouped[project_name or "Sem Projeto"].append(item)

    projects: list[ProjetoSummary] = []

    for project_name, rows in sorted(
        grouped.items(),
        key=lambda pair: pair[0].casefold(),
    ):
        introduction_dates = sorted({
            row.introduction_date
            for row in rows
            if row.introduction_date
        })
        storage_zones = sorted({
            row.storage_zone
            for row in rows
            if row.storage_zone
        })
        total_volume = round(
            sum(row.volume_calculado_periodo for row in rows),
            2,
        )
...
def parse_excel(
    ...
    all_missing = list(
        dict.fromkeys(absent_required + absent_optional)
    )
    ...
    for _, row in dataframe.iterrows():
        part_number_column = columns["part_number"]
        part_number = (
            _safe_str(row[part_number_column])
            if part_number_column
            else ""
        )

        if not part_number or part_number.casefold() in {
            "nan",
            "none",
            "null",
        }:
        ...
        description = (
            _safe_str(row[columns["descricao"]])
            if columns["descricao"]
            else ""
        )
        project = (
            _safe_str(row[columns["projeto"]])
            if columns["projeto"]
            else "Sem Projeto"
        ) or "Sem Projeto"
        introduction_date = (
            _safe_str(row[columns["introduction_date"]])
            if columns["introduction_date"]
            else ""
        )
        packaging_type = (
            _safe_str(row[columns["pckg_type"]])
            if columns["pckg_type"]
            else ""
        )
        storage_zone = (
            _safe_str(row[columns["storage_zone"]])
            if columns["storage_zone"]
            else ""
        )

        cxs_periodo = (
            _optional_nonnegative_float(row[columns["cxs_periodo"]])
            if columns["cxs_periodo"]
            else None
        )

        daily_rate_column = columns["daily_rate"]
        daily_rate = (
            _safe_float(row[daily_rate_column], default_daily_rate)
            if daily_rate_column
            else default_daily_rate
        )
        daily_rate = max(0.0, daily_rate)

        quantity_column = columns["qtde_pns_caixa"]
        quantity_per_box = (
            _safe_float(row[quantity_column], 1.0)
            if quantity_column
            else 1.0
        )
        ...
        if "daily_rate" in schema_fields:
            item_data["daily_rate"] = (
                daily_rate if daily_rate_column else None
            )

        if "qtde_pns_caixa" in schema_fields:
            item_data["qtde_pns_caixa"] = (
                quantity_per_box if quantity_column else None
            )

        if "volume_contratado" in schema_fields:
            item_data["volume_contratado"] = (
                _optional_nonnegative_float(
                    row[columns["volume_contratado"]]
                )
                if columns["volume_contratado"]
                else None
            )
    ...
    total_volume_periodo = round(
        sum(item.volume_calculado_periodo for item in items),
        2,
    )
    ...
    daily_rate_medio = (
        ...
        if actual_daily_rates
        else 0.0
    )

    total_volume_contratado = round(
        sum(
            float(getattr(item, "volume_contratado", 0.0) or 0.0)
            for item in items
        ),
        2,
    )
...
def list_projects() -> list[ProjetoSummary]:
    """Retorna os projetos do último upload."""
...
def _project_items(project: str) -> list[ProjFuturoItem]:
    ...
    return [
        ...
        if item.projeto.strip().casefold() == normalized_project
    ]
...
def simulate_project(req: SimulacaoRequest) -> SimulacaoResponse:
    ...
    if not all_items:
        return SimulacaoResponse(
            ...
            projeto=req.projeto,
            avisos=[
                "Nenhum arquivo carregado. Faça o upload primeiro."
            ],
        )
    ...
    usable_capacity = round(
        capacity * (1 - safety_margin_percent / 100),
        2,
    )
    projected_occupation = round(
        current_occupation + total_volume_periodo,
        2,
    )
    available_capacity = round(
        capacity - projected_occupation,
        2,
    )
    occupation_rate = round(
        (projected_occupation / capacity * 100)
        if capacity > 0
        else 0.0,
        2,
    )
    ...
    per_zone_result = {
        zone: round(volume, 2)
        for zone, volume in per_zone.items()
    }
    ...
    missing_volume_count = sum(
        1
        for item in items
        if item.calculo_fonte == "sem_dado"
    )
    ...
    if all(item.calculo_fonte == "sem_dado" for item in items):
        warnings.append(
            "Nenhuma peça do projeto possui Cxs/Periodo ou Daily Rate. "
            "O volume calculado é zero."
        )

    if projected_occupation > usable_capacity:
        exceeded = round(
            projected_occupation - usable_capacity,
            2,
        )
        warnings.append(
            "A demanda projetada excede a capacidade utilizável em "
            f"{exceeded:,.0f} caixas."
        )
