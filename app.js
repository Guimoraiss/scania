...
const PERIODS = [
  ...
  "JAN/2028","FEV/2028","MAR/2028","ABR/2028","MAI/2028","JUN/2028",
  "JUL/2028","AGO/2028","SET/2028","OUT/2028","NOV/2028","DEZ/2028",
  "JAN/2029","FEV/2029","MAR/2029","ABR/2029","MAI/2029","JUN/2029",
  "JUL/2029","AGO/2029","SET/2029","OUT/2029","NOV/2029","DEZ/2029",
  "JAN/2030","FEV/2030","MAR/2030","ABR/2030","MAI/2030","JUN/2030",
  "JUL/2030","AGO/2030","SET/2030","OUT/2030","NOV/2030","DEZ/2030",
];
...
const ZONE_DATA = {
  base10:       { name:"BASE 10",            baseOcc:82, capacity:68000,  note:"Armazenamento em pallet racks verticais — setor superior (BASE 10).",      color:"#4c1d95" },
  base20:       { name:"BASE 20",            baseOcc:79, capacity:55200,  note:"Armazenamento em pallet racks verticais — setor inferior (BASE 20).",      color:"#5b21b6" },
  blocado:      { name:"BLOCADO",            baseOcc:45, capacity:19800,  note:"Peças bloqueadas aguardando liberação de qualidade ou fiscal.",              color:"#6b21a8" },
  embalagem:    { name:"EMBALAGEM",          baseOcc:60, capacity:8400,   note:"Estação de embalagem e preparo de peças para envio.",                       color:"#166534" },
  recebimento:  { name:"RECEBIMENTO",        baseOcc:38, capacity:9600,   note:"Dock de recebimento e conferência de materiais.",                           color:"#1e40af" },
  bbpalletb10:  { name:"BLUE BOX BASE 10",   baseOcc:74, capacity:8000,   note:"Armazenamento de blue boxes em pallets — Base 10.",                         color:"#0e7490" },
  bbpalletb20:  { name:"BLUE BOX BASE 20",   baseOcc:74, capacity:8000,   note:"Armazenamento de blue boxes em pallets — Base 20.",                         color:"#0891b2" },
  bbindividual: { name:"BLUE BOX INDIVIDUAL",baseOcc:55, capacity:4200,   note:"Estoque de blue boxes individuais prontas para uso na linha.",              color:"#0f766e" },
  tmx:          { name:"T;M;4;0;X;90",       baseOcc:30, capacity:8000,   note:"Zona de armazenagem T;M;4;0;X;90.",                                         color:"#b45309" },
  expedicao:    { name:"EXPEDIÇÃO",          baseOcc:68, capacity:22000,  note:"Área de triagem e expedição de caixas para a produção.",                    color:"#92400e" },
  kd:           { name:"KD",                 baseOcc:50, capacity:5600,   note:"Kits de peças pré-montados para consumo direto na linha de produção.",      color:"#1d4ed8" },
  im:           { name:"I&M",                baseOcc:30, capacity:3200,   note:"Inspeção de qualidade e manutenção de embalagens retornáveis.",             color:"#991b1b" },
};
...
/* =========================================================================
   CAPACIDADES E OCUPAÇÕES POR ZONA
========================================================================= */
...
function calcTotalsFromZones() {
  const cap = getZoneCapacities();
  const occ = getZoneOccupied();
  const capacity   = cap.portaPalletsB10 + cap.portaPalletsB20 + cap.blueBoxB10 + cap.blueBoxB20 + cap.blueBoxIndividual + cap.blocado + cap.tmx;
  const currentOcc = occ.portaPalletsB10 + occ.portaPalletsB20 + occ.blueBoxB10 + occ.blueBoxB20 + occ.blueBoxIndividual + occ.blocado + occ.tmx;
...
function syncZoneCapacitiesFromInputs() {
  ...
  const zoneOccMap = {
    base10:occ.portaPalletsB10, base20:occ.portaPalletsB20,
    bbpalletb10:occ.blueBoxB10, bbpalletb20:occ.blueBoxB20,
    bbindividual:occ.blueBoxIndividual, blocado:occ.blocado, tmx:occ.tmx,
  };
...
function _getZoneCurrentOccupied() {
  const occ = getZoneOccupied();
  return {
    "Base 10":occ.portaPalletsB10, "Base 20":occ.portaPalletsB20,
    "Blue Box Base 10":occ.blueBoxB10, "Blue Box Base 20":occ.blueBoxB20,
    "Blue Box Individual":occ.blueBoxIndividual, "Blocado":occ.blocado, "T;M;4;0;X;90":occ.tmx,
  };
}
...
function _syncFutureVolumeContext() {
  ...
  window.lcbZonesOverlay?.setOverlayContext?.({
    ...
    volumeAtual:  producaoHoje,
    volumeFuturo: producaoFutura,
  });
}
...
function initMenu() {
  ...
  menuToggle.addEventListener("click", () => {
    ...
    if (isMobile) {
      ...
      setTimeout(() => { if (_caChart) _caChart.resize(); if (_donutChart) _donutChart.resize(); }, 300);
    }
  });
  ...
  document.querySelectorAll(".nav-link").forEach(link => {
    link.addEventListener("click", e => { e.preventDefault(); const page = link.dataset.page; if (page) showPage(page); });
  });
...
/* =========================================================================
   TIMELINE
========================================================================= */
const _MONTH_TO_NUM = { JAN:1,FEV:2,MAR:3,ABR:4,MAI:5,JUN:6,JUL:7,AGO:8,SET:9,OUT:10,NOV:11,DEZ:12 };
...
function renderTimelineTicks(periodsOverride) {
  ...
  activePeriods.forEach((p, i) => {
    ...
    ticks.push({ pct:(i/Math.max(total-1,1))*100, label:periodToQuarterLabel(p), i });
  });
  if (ticks.length === 0) {
    activePeriods.forEach((p, i) => ticks.push({ pct:(i/Math.max(activePeriods.length-1,1))*100, label:p, i }));
  }
...
function calibrateTimelineFromRows(rows) {
  ...
  for (let k = minKey; k <= maxKey; k++) {
    ...
    if (month >= 1 && month <= 12 && year >= 2020) periods.push(`${MON_PT[month-1]}/${year}`);
  }
  if (periods.length < 2) return;
  State._activePeriods = periods;
  State.periodIndex    = 0;
  const rangeEl = el("timelineRange");
  if (rangeEl) { rangeEl.min=0; rangeEl.max=periods.length-1; rangeEl.value=0; }
...
function initMapControls() {
  el("timelineRange")?.addEventListener("input", e => {
    ...
    if (State.hasRealData && State.lastResult) syncWarehouseFromResult(State.lastResult);
    else updateWarehouseSimulated();
  });
  ...
  el("zoneModal")?.addEventListener("click", e => { if (e.target === el("zoneModal")) el("zoneModal").close(); });
  Tooltip.init();
  renderTimelineTicks();
  renderSliderProgress(State.periodIndex);
}
...
function updateWarehouseTriggerBadges(rates) {
  ...
  if (!container) return;
  const featured = [
    { id:"base10",label:"BASE 10" },{ id:"base20",label:"BASE 20" },
    { id:"bbpalletb10",label:"BB P. B10" },{ id:"bbpalletb20",label:"BB P. B20" },
  ];
...
function updateWarehouseTriggerSub(result) {
  ...
  const labels  = { ok:"✓ LCB suporta a demanda", warning:"⚠ Monitoramento necessário", critical:"✗ Capacidade insuficiente" };
...
function renderWarehouseOverlay(sourceRows, result) {
  ...
  const rows = Array.isArray(sourceRows) && sourceRows.length > 0 ? sourceRows : (Array.isArray(State.lastRows) ? State.lastRows : []);
  const activePeriods  = State._activePeriods || PERIODS;
  const selectedPeriod = el("selectedPeriod")?.textContent || activePeriods[State.periodIndex] || null;
  const project = State.selectedProject;
  overlay.setOverlayContext?.({
    selectedPeriod, project,
    thresholdMedium:STATUS_THRESHOLD.WARNING, thresholdHigh:STATUS_THRESHOLD.CRITICAL,
    zoneCurrentOccupied:_getZoneCurrentOccupied(),
    volumeAtual:  Number(el("caVolumeFuturo")?.value)   || 0,
    volumeFuturo: Number(el("productionVolume")?.value) || State.productionVolume || 0,
  });
  overlay.renderZoneOverlay(rows, { selectedPeriod, project, thresholdMedium:STATUS_THRESHOLD.WARNING, thresholdHigh:STATUS_THRESHOLD.CRITICAL });
}
...
function updateWarehouseSimulated() {
  ...
  const rows = Array.isArray(State.lastRows) && State.lastRows.length > 0 ? State.lastRows : buildMockRows(calcCA(inputs));
  renderWarehouseOverlay(rows, { selectedPeriod:PERIODS[State.periodIndex], project:State.selectedProject, thresholdMedium:STATUS_THRESHOLD.WARNING, thresholdHigh:STATUS_THRESHOLD.CRITICAL });
}
  ...
  openModal(btn) {
    ...
    body.innerHTML = `
      <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:16px">
        ...
        <span style="padding:3px 10px;border-radius:5px;font-size:.7rem;font-weight:700;letter-spacing:.06em;background:var(--color-${status}-bg);border:1.5px solid var(--color-${status}-border);color:var(--color-${status==="ok"?"ok-border":status})">${labels[status]}</span>
      </div>
      <dl style="display:flex;flex-direction:column">
        ${[["Zona",data.name],["Ocupação atual",fmt.pct(rate)],["Capacidade total",cap>0?fmt.boxes(cap):"Não configurado"],["Disponível",cap>0?fmt.boxes(avail):"—"]].map(([k,v])=>`
...
function buildRealZoneRows(rows, result) {
  ...
  const backendByName = new Map(backendZones.map(zone => [String(zone?.zona??"").trim().toUpperCase(), zone]));

  (rows||[]).forEach(row => {
    const name = drawerText(row.storage_zone??row.storageZone??row.zona,"Sem Zona");
    const key  = name.toUpperCase();
    if (!groups.has(key)) groups.set(key,{id:key.toLowerCase().replace(/[^a-z0-9]+/g,"-"),name,itemCount:0,volumePD:0,contracted:0,blocked:0,stockValue:0});
  ...
  if (groups.size===0 && result?.porZona && typeof result.porZona==="object") {
    Object.entries(result.porZona).forEach(([name,value]) => {
      ...
      groups.set(key,{id:key.toLowerCase().replace(/[^a-z0-9]+/g,"-"),name:drawerText(name,"Sem Zona"),itemCount:0,volumePD:drawerNumber(value),contracted:0,blocked:0,stockValue:0});
    });
  }
  ...
  const MANUAL_CAPACITIES = {
    ...
    "BLUE BOX BASE 10":    Number(el("caLcbCapBlueBoxB10")?.value)        || 0,
    "BLUE BOX BASE 20":    Number(el("caLcbCapBlueBoxB20")?.value)        || 0,
    "BLUE BOX INDIVIDUAL": Number(el("caLcbCapBlueBoxIndividual")?.value) || 0,
    "BLOCADO":             Number(el("caLcbCapBlocado")?.value)           || 0,
    "T;M;4;0;X;90":       Number(el("caLcbCapTMX")?.value)               || 0,
  };

  const MANUAL_OCCUPIED = {
    "BASE 10":             Number(el("caCapPortaPalletsB10")?.value)    || 0,
    "BASE 20":             Number(el("caCapPortaPalletsB20")?.value)    || 0,
    "BLUE BOX BASE 10":    Number(el("caCapBlueBoxB10")?.value)         || 0,
    "BLUE BOX BASE 20":    Number(el("caCapBlueBoxB20")?.value)         || 0,
    "BLUE BOX INDIVIDUAL": Number(el("caCapBlueBoxIndividual")?.value)  || 0,
    "BLOCADO":             Number(el("caCapBlocado")?.value)            || 0,
    "T;M;4;0;X;90":       Number(el("caCapTMX")?.value)                || 0,
  };

  return [...groups.entries()].map(([key,group]) => {
    const backend     = backendByName.get(key);
    const capacity    = drawerNumber(backend?.total_slots) || MANUAL_CAPACITIES[key] || drawerNumber(group.contracted);
    const occupied    = drawerNumber(backend?.slots_usados) || MANUAL_OCCUPIED[key]  || drawerNumber(group.volumePD);
    const blocked     = drawerNumber(backend?.slots_bloqueados,group.blocked);
    const hasCapacity = capacity > 0;
    const available   = backend?.slots_disponiveis!=null ? drawerNumber(backend.slots_disponiveis) : hasCapacity?Math.max(0,capacity-occupied-blocked):null;
    const rate        = backend?.ocupacao_pct!=null ? drawerNumber(backend.ocupacao_pct) : hasCapacity?((occupied+blocked)/capacity)*100:null;
    const status      = backend?.risk_level==="HIGH"?"critical":backend?.risk_level==="MEDIUM"?"warning":rate==null?"ok":statusByRate(rate);
    return { ...group, capacity, occupied, blocked, available, rate, status, hasCapacity };
  }).sort((a,b) => { const rA=a.rate==null?-1:a.rate; const rB=b.rate==null?-1:b.rate; return rB-rA||b.volumePD-a.volumePD; });
}
...
const Drawer = {
  ...
  _renderContent(rows, result) {
    ...
    if (metaEl) {
      ...
      const sc = { ok:{bg:"var(--color-ok-bg)",border:"var(--color-ok-border)",text:"var(--color-ok)"}, warning:{bg:"var(--color-warning-bg)",border:"var(--color-warning-border)",text:"var(--color-warning)"}, critical:{bg:"var(--color-critical-bg)",border:"var(--color-critical-border)",text:"var(--color-critical)"} }[status];
      metaEl.innerHTML = `<div class="drawer-meta__chips">
        <span class="drawer-meta__chip"><span class="drawer-meta__chip-label">Projeto</span><span class="drawer-meta__chip-value" style="color:${project.color}">${escapeDrawerHtml(projectLabel)}</span></span>
        <span class="drawer-meta__chip"><span class="drawer-meta__chip-label">Itens analisados</span><span class="drawer-meta__chip-value">${rows.length}</span></span>
        <span class="drawer-meta__chip"><span class="drawer-meta__chip-label">Ocupação projetada</span><span class="drawer-meta__chip-value">${fmt.int(Math.round(projOcc))} cx</span></span>
        <span class="drawer-meta__chip"><span class="drawer-meta__chip-label">Disponível</span><span class="drawer-meta__chip-value">${fmt.int(Math.round(avail))} cx</span></span>
        <span class="drawer-meta__chip drawer-meta__chip--status" style="background:${sc.bg};border-color:${sc.border};color:${sc.text}">${{ok:"✓ SUPORTA",warning:"⚠ ATENÇÃO",critical:"✗ INSUFICIENTE"}[status]} · ${fmt.dec(occRate)}%</span>
      </div>`;
    }
    const bodyEl = el("drawerBody");
    if (!bodyEl) return;
    const zoneRows = buildRealZoneRows(rows, result);
    const dot = s => ({ok:`<span class="dz-dot dz-dot--ok"></span>`,warning:`<span class="dz-dot dz-dot--warn"></span>`,critical:`<span class="dz-dot dz-dot--crit"></span>`}[s]??"");
    const zoneSection = `
      <div class="drawer-section">
        <h3 class="drawer-section__title">Distribuição Real por Zona de Armazenagem</h3>
        ${zoneRows.length
          ? `<div class="drawer-zones">${zoneRows.map(zone => {
    ...
    if (!rows?.length) { bodyEl.innerHTML=zoneSection+`<div class="drawer-empty"><span class="drawer-empty__icon">📭</span><p>Nenhum item disponível.</p></div>`; return; }
    ...
    const totExcel      = rows.reduce((s,r)=>{const raw=r.cxs_periodo;const calc=drawerNumber(r.calc,r.volume_calculado_periodo);return s+(raw!=null&&raw!==""?drawerNumber(raw):calc);},0);
    bodyEl.innerHTML = zoneSection+`
      <div class="drawer-section">
        <h3 class="drawer-section__title">Itens Analisados</h3>
        <div class="drawer-table-wrap">
          <table class="drawer-table">
            <thead><tr><th>PN</th><th>Descrição</th><th>PE</th><th class="num">Vol. Excel</th><th class="num">Vol. Calculado</th><th>Zona</th><th>Data Intro</th><th class="num">Daily Rate</th><th class="num">Vol. Contratado</th><th class="num">Bloqueado</th><th>Origem</th><th>Fonte</th><th class="num">Impacto</th></tr></thead>
            <tbody>${rows.map(row=>{
  ...
  exportCSV(rows) {
    ...
    const csv = [
      ...
      ...rows.map(row=>[row.pn??row.part_number??"",row.desc??row.descricao??"",row.pe??row.pckg_type??"",row.cxs_periodo!=null&&row.cxs_periodo!==""?drawerNumber(row.cxs_periodo):drawerNumber(row.calc,row.volume_calculado_periodo),drawerNumber(row.calc,row.volume_calculado_periodo),row.storage_zone??"",row.introduction_date??"",drawerNumber(row.dr,row.daily_rate),drawerNumber(row.volC,row.volume_contratado),drawerNumber(row.bloqueado),row.origem??"",row.calculo_fonte??""].map(clean).join(";")),
    ];
    const blob = new Blob(["\uFEFF"+csv.join("\n")],{type:"text/csv;charset=utf-8;"});
    const url  = URL.createObjectURL(blob);
    const a    = document.createElement("a");
    a.href=url; a.download=`lcb_${State.selectedProject}_${Date.now()}.csv`; a.click();
...
function initDrawer() {
  ...
  el("btnOpenDrawer")?.addEventListener("click",   () => { if (State.lastResult) Drawer.open(State.lastRows, State.lastResult); });
...
function getCAInputs() {
  ...
  return {
    ...
    periodDays:15, dailyRate:7,
    capPortaPalletsB10:cap.portaPalletsB10, capPortaPalletsB20:cap.portaPalletsB20,
    capBlueBoxB10:cap.blueBoxB10, capBlueBoxB20:cap.blueBoxB20,
    capBlueBoxIndividual:cap.blueBoxIndividual, capBlocado:cap.blocado, capTMX:cap.tmx,
    occPortaPalletsB10:occ.portaPalletsB10, occPortaPalletsB20:occ.portaPalletsB20,
    occBlueBoxB10:occ.blueBoxB10, occBlueBoxB20:occ.blueBoxB20,
    occBlueBoxIndividual:occ.blueBoxIndividual, occBlocado:occ.blocado, occTMX:occ.tmx,
    volumeAtual: Number(el("caVolumeFuturo")?.value) || 0,
  };
}
...
function calcCA(inputs, totalProjectionOverride) {
  ...
  const factor          = ({ todos:1, china:1.0, europa:0.85, bev:1.2, powertrain:0.95 })[State.selectedProject] ?? 1;
  const volumePeriod    = State.productionVolume * periodDays;
  const totalProjection = totalProjectionOverride!=null ? Math.round(totalProjectionOverride) : Math.round(volumePeriod*dailyRate*factor);
  ...
  const occRate         = capacity>0?Math.min((projectedOcc/capacity)*100,200):200;
  const status          = available<0?"critical":statusByRate(occRate);
  return {
    volumePeriod:totalProjection, totalProjection, capacity, currentOcc,
    usableCapacity, projectedOcc, available, occRate, status, safetyAbs, periodDays, dailyRate,
    capPortaPalletsB10:inputs.capPortaPalletsB10, capPortaPalletsB20:inputs.capPortaPalletsB20,
    capBlueBoxB10:inputs.capBlueBoxB10, capBlueBoxB20:inputs.capBlueBoxB20,
    capBlueBoxIndividual:inputs.capBlueBoxIndividual, capBlocado:inputs.capBlocado, capTMX:inputs.capTMX,
    occPortaPalletsB10:inputs.occPortaPalletsB10, occPortaPalletsB20:inputs.occPortaPalletsB20,
    occBlueBoxB10:inputs.occBlueBoxB10, occBlueBoxB20:inputs.occBlueBoxB20,
    occBlueBoxIndividual:inputs.occBlueBoxIndividual, occBlocado:inputs.occBlocado, occTMX:inputs.occTMX,
    volumeAtual:inputs.volumeAtual, volumeFuturo:inputs.volumeAtual,
  };
}
...
function buildMockRows(result) {
  ...
  ].map(r=>({...r,calc:Math.round(r.dr*result.periodDays),cxs_periodo:Math.round(r.dr*result.periodDays),projeto:State.selectedProjectName||State.selectedProject}));
}
...
function renderDonutChart(result) {
  ...
  if (!canvas||typeof Chart==="undefined") { if (typeof Chart==="undefined") setTimeout(()=>renderDonutChart(result),200); return; }
  ...
  const margin  = result.safetyAbs!=null ? Math.round(result.safetyAbs) : Math.round(cap*((Number(el("caSafetyMargin")?.value)||0)/100));
...
function renderEngine(result) {
  ...
  const rClass  = status==="critical"?"ca-engine-step--crit":status==="warning"?"ca-engine-step--warn":"ca-engine-step--ok";
  const step = (tag,value,label) => `<div class="ca-engine-step"><span class="ca-engine-step__tag">${tag}</span><span class="ca-engine-step__value">${fmt.int(value)}</span><span class="ca-engine-step__label">${label}</span></div>`;
  const op   = (sym,eq=false) => `<div class="ca-engine-op${eq?" ca-engine-op--eq":""}">${sym}</div>`;
  container.innerHTML = `<div class="ca-engine">${step("CAPACIDADE",cap,"Σ Capacidade por Zona")}${op("−")}${step("OCUPAÇÃO",currOcc,"Σ Ocupação por Zona")}${op("−")}${step("MARGEM",margin,"Margem de Segurança")}${op("−")}${step("VOLUME PD",volPD,"Aumento PD Analisado")}${op("=",true)}<div class="ca-engine-step ${rClass}"><span class="ca-engine-step__tag">RESULTADO</span><span class="ca-engine-step__value">${fmt.int(avail)}</span><span class="ca-engine-step__label">Capacidade Disponível</span></div></div>`;
}
...
function renderCAVerdict(result) {
  const verdict=el("caVerdict"),icon=el("caVerdictIcon"),title=el("caVerdictTitle"),sub=el("caVerdictSub");
  if (!verdict) return;
...
function renderCAKpis(result) {
  ...
  if (!grid) return;
  const cap=result.capacity, currOcc=result.currentOcc;
  const avail=result.available??(cap-currOcc), occRate=result.occRate??0;
  const status=result.status??statusByRate(occRate);
  const kSC=status==="critical"?"ca-kpi--crit":status==="warning"?"ca-kpi--warn":"ca-kpi--ok";
  ...
  const occFuturaGlobal = (producaoHoje>0&&cap>0)?Math.min(((currOcc*producaoFutura/producaoHoje)/capacidadeTotal)*100,200):null;
  ...
  const occFuturaClass  = occFuturaGlobal!==null?(occFuturaStatus==="critical"?"ca-kpi--crit":occFuturaStatus==="warning"?"ca-kpi--warn":"ca-kpi--ok"):"";
  const availDisplay = avail<0?`−${fmt.int(Math.abs(Math.round(avail)))}`:fmt.int(Math.round(avail));
  const availDesc    = avail<0?`Excedido em ${fmt.int(Math.abs(Math.round(avail)))} cx`:"Folga após introdução do PD";
  const kpi = (label,value,unit,desc,iconKey,cls="") => `<div class="ca-kpi ${cls}"><div class="ca-kpi__header"><span class="ca-kpi__label">${label}</span><span class="ca-kpi__icon">${ICONS[iconKey]??""}</span></div><strong class="ca-kpi__value">${value}</strong><span class="ca-kpi__unit">${unit}</span><span class="ca-kpi__desc">${desc}</span></div>`;
  grid.innerHTML =
...
function renderCARecommendation(result) {
  const panel=el("caRecommendationPanel"),textEl=el("caRecommendationText");
  if (!textEl) return;
  ...
  const titles={ok:`O LCB suporta a demanda projetada${pLabel}`,warning:`Atenção: cenário${pLabel} exige monitoramento`,critical:`Capacidade insuficiente para a demanda${pLabel}`};
  const texts={ok:"O LCB suporta a demanda projetada com folga operacional. O cenário é favorável para a introdução planejada no período definido.",warning:"O cenário está na faixa de atenção. A operação é viável, mas é necessário monitorar as introduções e o uso das áreas de armazenagem ao longo do período.",critical:"A demanda projetada ultrapassa a capacidade utilizável do LCB. Recomenda-se redistribuição de volume, revisão da janela de introdução ou expansão de capacidade física."};
  const s=result.status;
  const iconEl=el("caRecommendationIcon"),titleEl=el("caRecommendationTitle");
...
function _showResultPanels() {
  const ph=el("caResultPlaceholder"),cards=el("caResultCards");
  ...
  window.lcbApi?.runIfFileLoaded(
    (calcResult) => {
      ...
      State.lastRows = rawItems.map((item) => ({
        ...
        bloqueado:item.bloqueado??0, origem:item.origem??"",
        calculo_fonte:item.calculo_fonte??"", valor_total:item.valor_total??0,
      }));
      ...
      const resultForRender = {
        status:localCalc.status, occRate:localCalc.occRate, available:localCalc.available,
        safetyAbs:localCalc.safetyAbs, projectedOcc:localCalc.projectedOcc,
        usableCapacity:localCalc.usableCapacity, volumePeriod:localCalc.volumePeriod,
        totalProjection:localCalc.totalProjection, capacity:localCalc.capacity,
        currentOcc:localCalc.currentOcc, increaseNeeded:calcResult.increaseNeeded,
        projectName:simulation.projeto||"", porZona:simulation.por_zona||{},
...
function initAnalise() {
  ...
  el("btnUseMock")?.addEventListener("click", () => {
    const capDefaults = { caLcbCapPortaPalletsB10:35766,caLcbCapPortaPalletsB20:19193,caLcbCapBlueBoxB10:0,caLcbCapBlueBoxB20:0,caLcbCapBlueBoxIndividual:0,caLcbCapBlocado:0,caLcbCapTMX:0,caSafetyMargin:10 };
    const occDefaults = { caCapPortaPalletsB10:24671,caCapPortaPalletsB20:13851,caCapBlueBoxB10:0,caCapBlueBoxB20:0,caCapBlueBoxIndividual:0,caCapBlocado:0,caCapTMX:0,caVolumeFuturo:0 };
    Object.entries({...capDefaults,...occDefaults}).forEach(([id,v])=>{ const e=el(id); if(e) e.value=v; });
    syncZoneCapacitiesFromInputs();
    runLocalCACalculation();
  });
  const capIds = ["caLcbCapPortaPalletsB10","caLcbCapPortaPalletsB20","caLcbCapBlueBoxB10","caLcbCapBlueBoxB20","caLcbCapBlueBoxIndividual","caLcbCapBlocado","caLcbCapTMX","caSafetyMargin"];
  const occIds = ["caCapPortaPalletsB10","caCapPortaPalletsB20","caCapBlueBoxB10","caCapBlueBoxB20","caCapBlueBoxIndividual","caCapBlocado","caCapTMX","caVolumeFuturo"];
  ...
  const uploadZone=el("uploadZone"),excelInput=el("excelInput"),uploadLabel=el("uploadLabel");
  function applyFile(file) {
    if (!file) return;
    State._timelineCalibrated=false; State._activePeriods=null;
  ...
  uploadZone?.addEventListener("drop", e => {
    ...
    if (excelInput) { excelInput.files=dt.files; excelInput.dispatchEvent(new Event("change",{bubbles:true})); }
  });
}
